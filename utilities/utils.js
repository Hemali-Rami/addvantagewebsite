...
export const normalizeResponseWithPagination = ({ response, key = "id" }) => {
  const dataArr = response.data.data.docs ? response.data.data.docs : [];

  //Current workaround

  let resPagination = {
    total: verifyObject(response, "data.data.totalDocs", 0),
    total_pages: verifyObject(response, "data.data.totalPages", 0),
    previous_page: verifyObject(response, "data.data.prevPage", 0),
    next_page: verifyObject(response, "data.data.nextPage", 0),
    limit: verifyObject(response, "data.data.limit", 0),
    hasNextPage: verifyObject(response, "data.data.hasNextPage", 0),
    hasPrevPage: verifyObject(response, "data.data.hasPrevPage", 0),
    currentPage: verifyObject(response, "data.data.pagingCounter", 0),
  };

  const { data } = normalize({ data: dataArr });
